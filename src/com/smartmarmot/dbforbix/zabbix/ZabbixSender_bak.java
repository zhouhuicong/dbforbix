 ...
 */
public class ZabbixSender_bak extends Thread {

	public enum PROTOCOL {
		V14, V18
	}

	private static final Logger	LOG					= Logger.getLogger(ZabbixSender_bak.class);
	...
	public ZabbixSender_bak(PROTOCOL protVer) {
		super("ZabbixSender");
		switch (protVer) {
			default:
				...
				for (Config.ZServer serverConfig : servers) {
					...
					for (int i = 0; i < 3; i++) {
						try {
							...
							LOG.debug("ZabbixSender data"+data + "  ---serverConfig.getHost() "+serverConfig.getHost());
							...
							if (!protocol.isResponeOK(read, response))
								LOG.warn("ZabbixSender - received unexpected response '" + new String(response).trim() + "' for key '" + nextItem.getKey()
										+ "'");
//							LOG.debug("ZabbixSender - received response '" + new String(response).trim() + "' for key '" + nextItem.getKey()
//							+ "'");
							break;
						}
						catch (Exception ex) {
